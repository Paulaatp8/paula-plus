...
function renderWatchlist(){
    ...
    allMovies = movies;
    getMovies()
    .filter(movie => movie.streamingServices)
    .slice(0,5)
    getMovies()
    .filter(movie => !movie.streamingServices)
    .length
...
function renderMovieGrid(movies){
    ...
    ${(
    getCurrentLanguage() === "es"
        ? (movie.overview_es || movie.overview || t("noSynopsis"))
        : (movie.overview_en || movie.overview || t("noSynopsis"))
).slice(0,240)}
${(
    (
        getCurrentLanguage() === "es"
...
async function migrateMovies(){
    ...
    for(const movie of movies){
        try{
            const [providers, es, en] =
                await Promise.all([
                    getWatchProviders(
                        movie.id
                    ),
                    fetch(
                        `https://api.themoviedb.org/3/movie/${movie.id}?language=es-ES`,
                        {
                            headers:{
                                Authorization:`Bearer ${CONFIG.TMDB_TOKEN}`,
                                "Content-Type":"application/json"
            ...
            movie.title_es =
                es.title || movie.title;
            movie.title_en =
                en.title || movie.title;
            movie.overview_es =
                es.overview || movie.overview;
            movie.overview_en =
                en.overview || movie.overview;
            updated++;
            console.log(
